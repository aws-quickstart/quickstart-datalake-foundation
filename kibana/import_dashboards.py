...
ELASTICSEARCH_TEMPLATE_URL = 'https://{es_endpoint}/{index_name}'
KIBANA_DASHBOARDS_PATH = os.path.join(PROJECT_DIR, 'kibana/kibana-visualizations.json')
ELASTICSEARCH_MAPPINGS_PATH = os.path.join(PROJECT_DIR, 'kibana/elasticsearch-mappings.json')
DEFAULT_INDEX_NAME = 'metadata'
INDEX_TO_TIME_FIELD = {
...
def register_kibana_indexes(elasticsearch_endpoint):
    ...
    send_kibana_put(elasticsearch_endpoint, type='config', id='5.1.1', data={'defaultIndex': DEFAULT_INDEX_NAME})
...
def import_visualizations(elasticsearch_endpoint):
    for visualization_json in load_json(KIBANA_DASHBOARDS_PATH):
        send_kibana_put(
